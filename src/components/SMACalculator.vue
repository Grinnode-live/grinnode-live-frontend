  ...
  <v-row justify="center">
        ...
        <v-row>
          <v-col cols="6">
            <v-btn elevation="0" color="primary" outlined
                   @click="onCalculateClick"> Calculate
            </v-btn>
          </v-col>
        </v-row>
      </v-container>
    ...
    <v-col cols="12" md="6" v-if="!error" align="center">
      ...
      <div class="flex  headline pt-2">
        <span class="flex-grow-1   "> {{ dividedCurrencyAmount|amount_filter }}  {{
      ...
      <div class="flex headline" v-if="dividedGrinCurrenyAmount>0">
        <span class="flex-grow-1 flex- "> {{ dividedGrinAmount | amount_filter }}  {{
            currency| currency_filter
          }}  </span>
        <span class="flex-grow-0"> = </span>
        <span class="flex-grow-1"> <span class="display-1 blue--text">{{
            dividedGrinCurrenyAmount | amount_filter
          }}</span> Grin</span>
      </div>
...
export default {
  ...
  computed: {
    endDateLong: function () {
      return (new Date(this.selectedDate)).getTime()
    },
    startDateLong: function () {
      return (new Date(this.selectedDate)).getTime() - this.monthPeriod * 30 * 24 * 60 * 60 * 1000
    },
    ...
    dividedCryptoAmount: function () {
      return this.dividedCurrencyAmount / this.avgCryptoPrice
    },
    ...
    dividedGrinCurrenyAmount: function () {
      return this.dividedGrinAmount / this.avgGrinPrice
    },
  },
  mounted: function () {
